require(["jquery","splunkjs/mvc/simplexml/ready!"],function($){
  ...
  function render(){
    ...
    $("#health-evidence-status").toggleClass("error",!!loadError).toggleClass("ready",!loadError&&records.length>0).text(loadError||(records.length?"Loaded "+records.length+" current lifecycle record"+(records.length===1?"":"s")+".":"No lifecycle evidence exists yet. Run discovery, build a detection, and validate it."));
  }
  function load(){var Store=window.DEILifecycleStore;loadError="";if(!Store||!Store.load){records=[];loadError="Detection health storage is unavailable. Verify DEI installation and permissions, then retry.";render();return;}$("#health-refresh").prop("disabled",true).attr("aria-busy","true").text("Refreshing…");Store.load().done(function(data){records=Array.isArray(data)?data:[];if(Store.mode&&Store.mode()!=="Splunk KV Store")loadError="Shared detection health could not load. Showing a non-durable browser recovery copy.";}).fail(function(){records=[];loadError="Detection health could not load. Verify DEI permissions and retry.";}).always(function(){$("#health-refresh").prop("disabled",false).removeAttr("aria-busy").text("Refresh health");render();});}

require(["jquery", "splunkjs/mvc/simplexml/ready!"], function ($) {
  ...
  function handle(action) {
    ...
    if (action==="deploy") {
      ...
      copy.catalog=$.extend({},copy.catalog,{status:production?"enabled":environment,enabled_at:production?now:null,enabled_by:production?Store.username():null}); copy=Store.appendHistory(copy,production?"catalog_detection_enabled":"nonproduction_deployment_recorded",target+" / "+environment+": "+external+(note?" · "+note:"")); save(copy,production?"Detection enabled and advanced to Production.":"Detection recorded in the "+label(environment)+" bucket. Promote it here when change control is complete."); return;
    }

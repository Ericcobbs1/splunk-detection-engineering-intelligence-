require(["jquery", "splunkjs/mvc/simplexml/ready!"], function ($) {
  ...
  function renderDetectionList() {
    ...
    $("#mitre-detection-list").html(items.length ? items.map(function (item) {
      return '<button type="button" class="dei-advisor-item ' + (selected && selected.detection_id === item.detection_id ? "selected" : "") + '" data-detection="' + escapeHtml(item.detection_id) + '"><div><span class="dei-severity ' + escapeHtml(item.severity) + '">' + escapeHtml(item.severity) + '</span><span class="dei-readiness ' + escapeHtml(item.readiness) + '">' + escapeHtml(String(item.readiness).replace(/_/g," ")) + '</span></div><strong>' + escapeHtml(item.name) + '</strong><small>' + escapeHtml((item.mitre_techniques || []).map(function (id) { return techniqueMap[id] && techniqueMap[id].currentId ? techniqueMap[id].currentId : id; }).join(" · ") || "No ATT&CK mapping") + '</small></button>';

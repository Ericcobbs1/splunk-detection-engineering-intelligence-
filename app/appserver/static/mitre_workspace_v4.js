require(["jquery", "splunkjs/mvc/simplexml/ready!"], function ($) {
  ...
  function renderPortfolioHeatmap() {
    var aggregate={};
    heatmapRecommendations().forEach(function(item){
      (item.mitre_techniques||[]).forEach(function(id){
        var current=techniqueMap[id]&&techniqueMap[id].currentId?techniqueMap[id].currentId:id;
        var entry=aggregate[current]||(aggregate[current]={count:0,ready:0,partial:0,gap:0,names:[]});
        entry.count+=1;entry.names.push(item.name);
        if(item.readiness==="production_ready"){entry.ready+=1;}
        else if(item.readiness==="field_gap"||item.readiness==="field_unverified"){entry.gap+=1;}
        else{entry.partial+=1;}
      });
    });
    $("#mitre-heatmap").html(tactics.map(function(tactic){
      var ids=Object.keys(techniqueMap).filter(function(id){return techniqueMap[id].tactics.indexOf(tactic.id)!==-1;}).map(function(id){return techniqueMap[id].currentId||id;});
      ids=ids.filter(function(id,index){return ids.indexOf(id)===index;}).sort();
      var cells=ids.map(function(id){
        var entry=aggregate[id]||{count:0,ready:0,partial:0,gap:0,names:[]};
        var level=entry.count===0?"none":entry.gap>0?"gap":entry.ready===entry.count?"strong":"partial";
        var meta=techniqueMap[id]||{};
        return '<button type="button" class="dei-heat-technique" data-technique="'+escapeHtml(id)+'" data-level="'+level+'" title="'+escapeHtml(entry.names.join(" · ")||"No mapped detections")+'"><span>'+escapeHtml(id)+' · '+escapeHtml(meta.name||"Technique")+'</span><b>'+entry.count+'</b></button>';
      }).join("");
      return '<article class="dei-heat-tactic"><strong>'+escapeHtml(tactic.name)+'</strong><div class="dei-heat-techniques">'+cells+'</div></article>';
    }).join(""));
  }
  ...
  $("#mitre-detection-list").on("click", ".dei-advisor-item", function () {
    ...
    renderDetectionList(); renderMatrix(); renderPortfolioHeatmap(); renderInspector(selected);
    $(document).trigger("dei:advisor-detection-selected",[id]);
  });
  $(".dei-mitre-view-toggle").on("click","button",function(){
    var view=$(this).attr("data-view");$(".dei-mitre-view-toggle button").removeClass("active").attr("aria-selected","false");$(this).addClass("active").attr("aria-selected","true");
    $("#mitre-matrix").prop("hidden",view!=="matrix");$("#mitre-heatmap-panel").prop("hidden",view!=="heatmap");
  });
  $("#mitre-matrix, #mitre-heatmap, #mitre-inspector-body").on("click", "[data-technique]", function () {

require(["jquery", "splunkjs/mvc/simplexml/ready!"], function ($) {
  ...
  function fieldMarkup(record) {
    ...
    if (record.state==="production" || record.state==="monitoring") {
      ...
      return '<div class="dei-action-fields-row"><label class="dei-action-field"><span>Health *</span><select id="lifecycle-health"><option value="healthy"'+(prior.health==="healthy"?" selected":"")+'>Healthy</option><option value="degraded"'+(prior.health==="degraded"?" selected":"")+'>Degraded</option><option value="failing"'+(prior.health==="failing"?" selected":"")+'>Failing</option></select></label><label class="dei-action-field"><span>Review period *</span><input id="lifecycle-review-period" type="text" value="'+esc(prior.review_period||"Last 24 hours")+'" placeholder="Example: Last 24 hours"/></label><label class="dei-action-field"><span>Result volume *</span><input id="lifecycle-result-volume" type="number" min="0" value="'+esc(prior.result_volume||0)+'"/></label><label class="dei-action-field"><span>Runtime ms *</span><input id="lifecycle-runtime" type="number" min="0" value="'+esc(prior.runtime_ms||0)+'"/></label><label class="dei-action-field"><span>True positives</span><input id="lifecycle-true-positives" type="number" min="0" value="'+esc(prior.true_positives||0)+'"/></label><label class="dei-action-field"><span>False positives</span><input id="lifecycle-false-positives" type="number" min="0" value="'+esc(prior.false_positives||0)+'"/></label></div><label class="dei-action-field"><span id="lifecycle-action-comment-label">Operational or tuning rationale</span><textarea id="lifecycle-action-comment" aria-describedby="lifecycle-action-comment-help lifecycle-inline-error" placeholder="Document health context, the tuning objective, or the retirement reason."></textarea><small id="lifecycle-action-comment-help">A specific rationale is required to start Tuning or retire this detection.</small></label><div id="lifecycle-inline-error" class="dei-inline-action-error" role="alert" hidden="hidden"></div>';
    }
  ...
  function handleAction(action) {
    ...
    if (action==="record_health") {
      var health=String($("#lifecycle-health").val()||"healthy"); var reviewPeriod=String($("#lifecycle-review-period").val()||"").trim(); var volume=Number($("#lifecycle-result-volume").val()||0); var runtime=Number($("#lifecycle-runtime").val()||0); var truePositives=Number($("#lifecycle-true-positives").val()||0); var falsePositives=Number($("#lifecycle-false-positives").val()||0);
      if (!reviewPeriod) { actionError("Enter the review period represented by these measurements.","Review period *"); return; }

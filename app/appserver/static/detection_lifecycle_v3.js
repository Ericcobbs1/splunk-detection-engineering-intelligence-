require(["jquery", "splunkjs/mvc/simplexml/ready!"], function ($) {
  ...
  function fieldMarkup(record) {
    ...
    if (record.state==="production" || record.state==="monitoring") {
      ...
      return ownership+'<div class="dei-action-fields-row"><label class="dei-action-field"><span>Health *</span><select id="lifecycle-health"><option value="healthy"'+(prior.health==="healthy"?" selected":"")+'>Healthy</option><option value="degraded"'+(prior.health==="degraded"?" selected":"")+'>Degraded</option><option value="failing"'+(prior.health==="failing"?" selected":"")+'>Failing</option></select></label><label class="dei-action-field"><span>Review period *</span><input id="lifecycle-review-period" type="text" aria-describedby="lifecycle-inline-error" value="'+esc(prior.review_period||"Last 24 hours")+'" placeholder="Example: Last 24 hours"/></label><label class="dei-action-field"><span>Result volume *</span><input id="lifecycle-result-volume" type="number" min="0" aria-describedby="lifecycle-inline-error" value="'+esc(prior.result_volume||0)+'"/></label><label class="dei-action-field"><span>Runtime ms *</span><input id="lifecycle-runtime" type="number" min="0" aria-describedby="lifecycle-inline-error" value="'+esc(prior.runtime_ms||0)+'"/></label><label class="dei-action-field"><span>True positives</span><input id="lifecycle-true-positives" type="number" min="0" aria-describedby="lifecycle-inline-error" value="'+esc(prior.true_positives||0)+'"/></label><label class="dei-action-field"><span>False positives</span><input id="lifecycle-false-positives" type="number" min="0" aria-describedby="lifecycle-inline-error" value="'+esc(prior.false_positives||0)+'"/></label></div><label class="dei-action-field"><span id="lifecycle-action-comment-label">Monitoring evidence note *</span><textarea id="lifecycle-action-comment" aria-describedby="lifecycle-action-comment-help lifecycle-inline-error" placeholder="Record scheduler/search-history evidence, source-data freshness, analyst outcomes, downstream actions, and the next corrective action when needed."></textarea><small id="lifecycle-action-comment-help">Use real evidence. Explain zero results, degraded or failing health, and any corrective action. This note also supports a later tuning or retirement decision.</small></label><div id="lifecycle-inline-error" class="dei-inline-action-error" role="alert" hidden="hidden"></div>';
    }

require(["jquery", "splunkjs/mvc/simplexml/ready!"], function ($) {
  ...
  function guide(item) {
    ...
    var guides={
      recommendation:{title:buildable?"Build the first detection draft":"Resolve telemetry prerequisites",explanation:buildable?"The recommendation is available, but no versioned detection artifact exists yet.":"This recommendation is blocked until its required telemetry or field evidence is available.",why:buildable?"Starting in Builder converts telemetry and ATT&CK evidence into reviewable SPL without enabling anything.":"Resolving telemetry first prevents the user from generating SPL that cannot run correctly in this environment.",requirements:[["Telemetry readiness",buildable],["MITRE mapping",mitre(item).length>0],["Observed sourcetype",sources(item).length>0]],action:buildable?"Start detection draft":"Review telemetry actions",href:buildable?"detection_builder?detection="+id:"detection_action_center?category=telemetry",note:buildable?"Builder will open this recommendation and generate an editable draft.":"The Action Center will show the evidence that must be resolved before building."},
      draft:{title:"Complete and validate the SPL",explanation:"A draft exists. Review its search logic and run bounded historical validation before review.",why:"Validation proves that the SPL parses, runs safely, and produces reviewable evidence against the available telemetry.",requirements:[["Detection SPL",!!record.spl],["Schedule",!!record.schedule],["Validation passed",validation.status==="passed"]],action:validation.status==="passed"?"Continue to review handoff":"Open Builder and validate",href:validation.status==="passed"?"detection_operations?detection="+id:"detection_builder?detection="+id,note:"Only passed validation can advance to peer review."},
      testing:{title:validation.status==="passed"?"Submit for peer review":"Repair or rerun validation",explanation:validation.status==="passed"?"Validation passed. Add the review handoff note and submit this version for an independent decision.":"Testing is active, but passed validation evidence is still required.",why:"Peer review should evaluate a stable, tested version with documented intent and expected analyst behavior.",requirements:[["Validation passed",validation.status==="passed"],["Result evidence",Number(validation.result_count||0)>=0],["Review submission",review.decision==="pending"]],action:"Open validation handoff",href:"detection_operations?detection="+id,note:"Engineering Operations will show only the required review handoff fields."},
      peer_review:{title:"Complete the peer-review decision",explanation:"A reviewer must approve this version or return it with specific changes.",why:"Independent review prevents untested or poorly scoped logic from reaching the operational catalog.",requirements:[["Validation passed",validation.status==="passed"],["Submitted for review",review.decision==="pending"||review.decision==="approved"],["Peer approval",review.decision==="approved"]],action:"Open peer review",href:"detection_operations?detection="+id,note:"Approval automatically moves the detection out of the engineering queue and into the catalog."},
      ...
      tuning:{title:"Revise and revalidate the detection",explanation:"A new tuning version is active. Update the SPL and repeat validation and peer review.",why:"Operational changes must not inherit validation or approval from the prior version.",requirements:[["Tuning version",Number(record.version||1)>1],["Revised SPL",!!record.spl],["Fresh validation",validation.status==="passed"]],action:"Open Builder for tuning",href:"detection_builder?detection="+id,note:"The tuned version returns through validation and peer review before re-enablement."},

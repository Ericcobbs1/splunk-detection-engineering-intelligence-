require(["jquery", "splunkjs/mvc/simplexml/ready!"], function ($) {
  ...
  function renderChanges() {
    ...
    (changes.freshness_changes||[]).forEach(function(change){findings.push({state:change.current_freshness==="active"?"new":"changed",title:"Telemetry freshness changed: "+change.source,detail:change.detail});});
    ...
    (changes.readiness_regressions||[]).forEach(function(change){findings.push({state:"risk",title:"Detection readiness regressed",detail:change.name+" changed from production ready to "+change.current_readiness.replace(/_/g," ")+(change.cause==="telemetry_age_or_freshness"?" because recent telemetry may have aged outside the active window. Verify ingestion freshness before changing the detection design.":" and requires revalidation.")});});

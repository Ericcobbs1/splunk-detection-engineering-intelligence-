...
def test_customer_pages_load_shared_readable_design_system() -> None:
    for view_name in (
        ...
        "detection_lifecycle", "detection_catalog", "detection_catalog",
...
def test_operate_pages_do_not_duplicate_navigation_with_a_view_dropdown() -> None:
    for view_name in ("detection_catalog", "detection_catalog", "detection_lifecycle"):
        root = ElementTree.parse(VIEWS / f"{view_name}.xml").getroot()
        assert root.find(".//*[@id='lifecycle-workspace-menu']") is None
